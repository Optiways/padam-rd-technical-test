...
def dijkstra(graph: Graph, start: int, target: int) -> tuple[int, list[Edge]]:
    """Return the shortest path between start and target in graph using Dijkstra's algorithm"""
    if start == target:
        return dijkstra_path(graph, (0, start, -1))
    ...
    while open:
        ...
        closed.add(u)
        if u == target:
            return dijkstra_path(graph, (cost, u, prev_idx, prev))
    ...
    return dijkstra_path(graph, (0, None, -1))


def dijkstra_path(graph: Graph, last: tuple) -> tuple[int, list[Edge]]:
    """Rebuild the path from dijkstra search"""
    path = []
    cost = last[0]
    while last[2] != -1:
        path.append(graph.edge_map[(last[1], last[3][1], last[2])])
        last = last[3]
    path.reverse()
    return cost, path
